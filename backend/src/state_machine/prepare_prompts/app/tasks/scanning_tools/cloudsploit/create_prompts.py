...
from common.config import (
    CLOUDSPLOIT_JSON_PLACEHOLDER,
    CLOUDSPLOIT_OUTPUT_PATH,
    CLOUDSPLOIT_PROMPT_PATH,
    S3_BUCKET,
    STORE_CHUNK_PATH,
    STORE_PROMPT_PATH,
    WAFR_JSON_PLACEHOLDER,
)
from common.entities import CloudSploitFinding, Finding, FindingExtra, FindingResource, Prompt, PromptS3Uri
...
logger = logging.getLogger("CreateCloudSploitPrompts")
logger.setLevel(logging.DEBUG)

CHUNK_SIZE = 700


class CloudSploitCreatePrompts(CreatePromptsTask):
    ...
    def save_chunk_for_retrieve(
        ...
        key = STORE_CHUNK_PATH.format(assessment_id, f"cloudsploit-{chunk_index}")
    ...
    def retrieve_prompt(self) -> str:
        with Path(CLOUDSPLOIT_PROMPT_PATH).open() as f:
            return f.read()
    ...
    def store_prompts(
        ...
        for i, prompt in enumerate(prompts):
            key = STORE_PROMPT_PATH.format(assessment_id, f"cloudsploit-{i}")
