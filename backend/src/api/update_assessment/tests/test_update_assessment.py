...
def test_update_assessment():
    ...
    task_input = UpdateAssessmentInput(assessment_id="AID", owner_id="test-owner-id", assessment_dto=assessment_dto)
    task = UpdateAssessment(assessment_service)
    response = task.execute(task_input)

    assessment_service.update_assessment.assert_called_once_with(
        assessment_id="AID", owner_id="test-owner-id", assessment_dto=AssessmentDto(name="AN")
    )
