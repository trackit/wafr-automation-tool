...
import { PreparePromptsUseCaseImpl } from './PreparePromptsUseCase';
...
describe('PreparePrompts Use Case', () => {
  it('should throw an error if assessment is not found', async () => {
    ...
    const input = PreparePromptsUseCaseArgsMother.basic()
      ...
      .build();
    await expect(useCase.preparePrompts(input)).rejects.toThrow(NotFoundError);
  });
  ...
  it('should call StorePrompts with unmapped findings', async () => {
    const {
      useCase,
      getScannedFindingsUseCase,
      mapScanFindingsToBestPracticesUseCase,
  ...
  it('should forward StorePrompts return', async () => {
    const {
      useCase,
      getScannedFindingsUseCase,
      mapScanFindingsToBestPracticesUseCase,
    ...
    storePromptsUseCase.storePrompts.mockResolvedValue(URIs);
    ...
    const input = PreparePromptsUseCaseArgsMother.basic()
      ...
      .withScanningTool(ScanningTool.PROWLER)
      .build();
    const result = await useCase.preparePrompts(input);
...
const setup = () => {
  ...
  const storePromptsUseCase = { storePrompts: vi.fn() };
  ...
  register(tokenStorePromptsUseCase, {
    useValue: storePromptsUseCase,
  });
  return {
    getScannedFindingsUseCase,
    mapScanFindingsToBestPracticesUseCase,
    ...
    useCase: new PreparePromptsUseCaseImpl(),
  };
};
