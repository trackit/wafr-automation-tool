...
import { GetAssessmentVersionsAdapterEventMother } from './GetAssessmentVersionsAdapterEventMother';

describe('getAssessmentVersions adapter', () => {
  describe('args validation', () => {
    it('should validate args', async () => {
      const { adapter } = setup();

      const event = GetAssessmentVersionsAdapterEventMother.basic().build();
    ...
    it('should return a 400 status code with invalid assessmentId', async () => {
      ...
      const event = GetAssessmentVersionsAdapterEventMother.basic()
  ...
  describe('useCase and return value', () => {
    it('should call useCase with the correct parameters', async () => {
      ...
      expect(useCase.getAssessmentVersions).toHaveBeenCalledWith({
...
const setup = () => {
  ...
  const useCase = { getAssessmentVersions: vitest.fn() };
  useCase.getAssessmentVersions.mockResolvedValueOnce(
    Promise.resolve({ versions: [], nextToken: undefined }),
  );
  register(tokenGetAssessmentVersionsUseCase, { useValue: useCase });

  return { parseSpy, useCase, adapter: new GetAssessmentVersionsAdapter() };
};
